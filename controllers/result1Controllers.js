...
const resultSubmit = async (req, res) => {
  ...
  const quiz = await Quiz1.findById(quizId);
  if (!quiz) {
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    const sortedUserAnswer = userAnswer.slice().sort(); // Sort user's answer array
    const sortedCorrectAnswer = correctAnswer.slice().sort(); // Sort correct answer array
    const isCorrect = compareAnswers(sortedUserAnswer, sortedCorrectAnswer);
